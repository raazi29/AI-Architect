...
class ProkeralaAstrologyService:
    def __init__(self):
        ...
        if self.client_id and self.client_secret:
            print(
                f"[OK] Prokerala API credentials loaded - Client ID: {self.client_id[:8]}..."
            )
        else:
            print(
                "[ERROR] Prokerala API credentials not found in environment variables"
            )
            print(
                "Please set PROKERALA_CLIENT_ID and PROKERALA_SECRET_KEY in your .env file"
            )

    async def get_access_token(self) -> Optional[str]:
        """Get OAuth access token from Prokerala"""
        if not self.client_id or not self.client_secret:
            print("[ERROR] Cannot get access token: Missing Prokerala credentials")
            return None
        ...
        try:
            ...
            print(f"[INFO] Requesting Prokerala access token from {auth_url}")
            response = requests.post(auth_url, data=auth_data, timeout=15)

            if response.status_code == 200:
                result = response.json()
                self.access_token = result.get("access_token")
                print("[OK] Prokerala access token obtained successfully")
                return self.access_token
            else:
                print(
                    f"[ERROR] Prokerala auth failed: {response.status_code} - {response.text}"
                )
                return None

        except Exception as e:
            print(f"[ERROR] Error getting Prokerala access token: {e}")
            return None
    ...
    async def get_birth_chart(
        ...
        try:
            ...
            if not token:
                print("[ERROR] Cannot fetch birth chart: No access token available")
                return None
            ...
            try:
                ...
                print(
                    f"[DATE] Parsed birth details: {birth_dt.strftime('%Y-%m-%d')} at {hour:02d}:{minute:02d}"
                )
            except ValueError as e:
                print(f"[ERROR] Date/time parsing error: {e}")
                return None

            # Get coordinates
            coordinates = self.get_coordinates(birth_place)
            print(
                f"[LOCATION] Using coordinates for {birth_place}: {coordinates['lat']}, {coordinates['lng']}"
            )
            ...
            datetime_str = (
                f"{birth_dt.strftime('%Y-%m-%d')}T{hour:02d}:{minute:02d}:00+05:30"
            )
            ...
            print(f"[INFO] Requesting birth chart from Prokerala API...")
            response = requests.get(
                chart_url, params=params, headers=headers, timeout=20
            )

            if response.status_code == 200:
                data = response.json()
                print("[OK] Birth chart data received successfully from Prokerala")
                # Debug: Print response structure (first 500 chars)
                import json

                response_str = json.dumps(data, indent=2)[:500]
                print(f"[DATA] API Response preview: {response_str}...")
                return data
            else:
                print(
                    f"[ERROR] Prokerala birth chart API error: {response.status_code} - {response.text}"
                )
                return None

        except Exception as e:
            print(f"[ERROR] Error fetching birth chart from Prokerala: {e}")
            return None
    ...
    def extract_vastu_relevant_data(self, chart_data: Dict) -> Dict[str, Any]:
        ...
        if not chart_data or "data" not in chart_data:
            print(
                "[WARNING] No valid chart data available, cannot provide authentic astrological insights"
            )
            return None
        ...
        try:
            print("[SEARCH] Extracting astrological data from Prokerala response...")
            ...
            print(
                f"[STAR] Extracted signs - Sun: {sun_sign}, Moon: {moon_sign}, Ascendant: {ascendant}"
            )

            # Create a planets dict from the available data for calculations
            planets = {"sun": {"sign": sun_sign}, "moon": {"sign": moon_sign}}

            # Calculate favorable directions based on planetary positions
            favorable_directions = self.calculate_favorable_directions_from_planets(
                planets
            )
            ...
            print("[OK] Successfully extracted authentic astrological data")
            return result

        except Exception as e:
            print(f"[ERROR] Error extracting chart data: {e}")
            return None
    ...
    def calculate_favorable_directions_from_planets(self, planets: Dict) -> List[str]:
        ...
        try:
            ...
            print(
                f"[DIRECTION] Calculated favorable directions from planets: {directions}"
            )
    ...
    def calculate_lucky_numbers_from_planets(self, planets: Dict) -> List[int]:
        ...
        try:
            ...
            print(
                f"[NUMBER] Calculated lucky numbers from planetary degrees: {numbers}"
            )
            return numbers[:5]
    ...
    def get_gemstone_recommendations_from_planets(self, planets: Dict) -> List[str]:
        ...
        try:
            ...
            if any(
                weak_sign in jupiter_sign
                for weak_sign in ["gemini", "virgo", "capricorn"]
            ):
            ...
            print(
                f"[GEM] Recommended gemstones based on planetary analysis: {gemstones}"
            )
            return gemstones[:3]
    ...
    async def get_authentic_astrology_data(
        self, birth_date: str, birth_time: str, birth_place: str
    ) -> Optional[Dict[str, Any]]:
        """Get authentic astrology data from Prokerala API - no fallbacks"""
        try:
            print(
                f"[MAGIC] Fetching authentic astrology data for {birth_place} on {birth_date} at {birth_time}"
            )
            ...
            if not chart_data:
                print("[ERROR] Failed to get birth chart from Prokerala API")
                return None
            ...
            if not vastu_data:
                print("[ERROR] Failed to extract astrological insights")
                return None

            print(
                "[OK] Successfully obtained authentic astrological data from Prokerala"
            )
            return vastu_data

        except Exception as e:
            print(f"[ERROR] Error getting authentic astrology data: {e}")
            return None

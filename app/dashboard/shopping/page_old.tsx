...
export default function SmartShopping() {
  ...
  const [shopCategory, setShopCategory] = useState("commercial.furniture")
  ...
  useEffect(() => {
    const fetchRealTimeFeatures = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json()
          setRealTimeFeatures(data)
        }
      } catch (err) {
        console.error("Error loading real-time features:", err)
      }
    }

    fetchRealTimeFeatures()
  }, [])
          ...
          <div className="bg-card border border-border rounded-lg p-6 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div className="grid grid-cols-2 gap-2">
                <Input placeholder="Latitude" value={lat} onChange={(e) => setLat(e.target.value === '' ? '' : Number(e.target.value))} />
                <Input placeholder="Longitude" value={lon} onChange={(e) => setLon(e.target.value === '' ? '' : Number(e.target.value))} />
              </div>
              <div>
                <Select value={shopCategory} onValueChange={setShopCategory}>
                  ...
                  <SelectContent>
                    <SelectItem value="commercial.furniture">Furniture</SelectItem>
                    <SelectItem value="commercial.electronics">Electronics</SelectItem>
                    <SelectItem value="commercial.hardware">Hardware</SelectItem>
                    <SelectItem value="commercial.appliances">Appliances</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
                <label className="text-sm text-foreground mb-2 block">Radius: {shopRadius} m</label>
                <Slider value={[shopRadius]} onValueChange={(v) => setShopRadius(v[0])} min={500} max={5000} step={100} />
              </div>
              <div className="flex gap-2">
                <Input placeholder="Enter city or area" value={placeText} onChange={(e) => setPlaceText(e.target.value)} />
                <Button variant="outline" onClick={geocodeAndSetCoords} disabled={geocoding}>{geocoding ? 'Searching…' : 'Search Location'}</Button>
                <Button variant="outline" onClick={useMyLocation}>Use My Location</Button>
                <Button onClick={fetchShops} disabled={loadingShops}>Find Nearby Shops</Button>
              </div>
            </div>
            {shopsError && (
              <div className="text-sm text-red-500 mb-3">{shopsError}</div>
            )}
            {loadingShops && (
                ...
                {shops.map((s) => (
                  <Card key={s.id} className="p-4">
                    <div className="flex items-start justify-between">
                      ...
                      {s.distance != null && (
                        <Badge variant="outline" className="text-xs">{Math.round(s.distance)} m</Badge>
                      )}
                    </div>
                    {s.opening_hours && (
                      <div className="mt-2 text-xs text-muted-foreground">Opening hours available</div>
                    )}
                  </Card>
                ))}
              </div>
            ...
            <div className="mt-4">
              <div className="h-80 rounded-lg overflow-hidden border" ref={mapContainerRef} />
            </div>
          </div>
        ...
        {realTimeFeatures && (
          <div className="mt-16 border-t border-border pt-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {realTimeFeatures.price_comparisons && realTimeFeatures.price_comparisons.length > 0 && (
                <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${realTimeFeatures.price_comparisons[1] ? (realTimeFeatures.price_comparisons[0].price / realTimeFeatures.price_comparisons[1].price) * 100 : 100}%` }}
                        ></div>
                      </div>
              ...
              {realTimeFeatures.trending_products && realTimeFeatures.trending_products.length > 0 && (
                <div>
            ...
            {realTimeFeatures.inventory_updates && realTimeFeatures.inventory_updates.length > 0 && (
              <div className="mt-8">
